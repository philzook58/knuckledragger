...
def declare(name: str, *sorts: smt.SortRef) -> smt.FuncDeclRef:
    """
...
def is_declared(e: smt.ExprRef | smt.FuncDeclRef) -> bool:
    if isinstance(e, smt.ExprRef):
        decl = e.decl()
    elif isinstance(e, smt.FuncDeclRef):
        decl = e
    else:
        raise TypeError("Expected ExprRef or FuncDeclRef", e)
    return __reserved_decls.get(decl.name()) == decl


@dataclass(frozen=True)
class Unfolding(Judgement):
    """
    A record storing definition. It is useful to record definitions as special axioms because we often must unfold them.
    It's more a recognition of a fact in unfoldable form
    """
...
# TODO: move out of kernel? Just useful as a tactic not as a source of truth
defns: dict[smt.FuncDeclRef, Unfolding] = {}
"""
...
def register_definition(defn: Unfolding):
    """
    Can register other unfoldings besides those generated by `define`.
    This may be useful to get unfolding for objects constructed in roundabout ays.
    """
    defns[defn.decl] = defn
...
def make_unfolding(pf: Proof) -> Unfolding:
    ...
    and return a Defn object for `f`.
    Unfolding judgments can be used with `unfold_defns`.
    ...
    f = lhs.decl()
    assert all(a.eq(v) for a, v in zip(lhs.children(), vs))
    ...
    return Unfolding(
        ...
        _subst_fun_body=smt.substitute(
            rhs, *[(a, smt.Var(n, a.sort())) for n, a in enumerate(vs)]
        ),
    )


# TODO: move define out of kernel?
# declare + axiom of definition + make_defn.
...
def unfold_defns(
    ...
    """
    assert all(isinstance(d, Unfolding) for d in defns)
    t = smt.substitute_funs(e, *[(d.decl, d._subst_fun_body) for d in defns])
    return t, kd.axiom(smt.Eq(e, t), by=["unfold_defns", defns])
...
def unfold(
    ...
    return unfold_defns(e, [defns[d] for d in decls])
