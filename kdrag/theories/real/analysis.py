...
is_open.full = kd.prove(is_open(RSet.full), unfold=1)
is_open.empty = kd.prove(is_open(RSet.empty), unfold=1)
...
l.auto(unfold=2)
is_open.unit_ball = l.qed()


dist = kd.define("dist", [x, y], real.abs(x - y))
dist.refl = kd.prove(kd.QForAll([x], dist(x, x) == 0), unfold=2)
dist.pos = kd.prove(smt.Implies(x != y, dist(x, y) > 0), unfold=2).forall([x, y])
dist.pos2 = kd.prove(dist(x, y) >= 0, unfold=2).forall([x, y])
