...
class BaseSolver:
    ...
    def write_tptp(self, filename, thf=True):
        ...
        with open(filename, "w") as fp:
            ...
            for sort in collect_sorts(
                ...
                if name not in ["Int", "Real", "Bool"]:
                    fp.write(
                        f"{tptp}({name.lower()}_type, type, {kd.utils.sort_to_tptp(sort)} : $tType ).\n"
                    )
            ...
            for f in collect_decls(
                ...
                if f not in predefined and f.name() not in self.predefined_names:
                    if f.arity() == 0:
                        fp.write(
                            f"{tptp}({f.name()}_type, type, {kd.utils.mangle_decl(f)} : {f.range().tptp()} ).\n"
                        )
                    else:
                        dom_tptp = " > ".join(
...
class TweeSolver(BaseSolver):
    def check(self):
        ...
        self.write_tptp(filename, thf=False)
